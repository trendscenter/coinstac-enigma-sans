WriteRawFiles <- function(merged_ordered, Ncol, outputdir, outputfilenamePrefix) {
...
for(z in (Ncol+1):ncol(merged_ordered)) {
  ...
  min.raw[z-Ncol]=min(!is.na(merged_ordered[,z]), na.rm=T)
  max.raw[z-Ncol]=max(!is.na(merged_ordered[,z]), na.rm=T)
}

#Save raw values
save(raw.means, sd.raw, n.raw, min.raw, max.raw, file=paste0(outputdir, outputfilenamePrefix,".Rdata"))
...
for (DXvalue in 0:1){
  ...
  for(z in (Ncol+1):ncol(merged_ordered)){
    ...
    min.raw[z-Ncol]=min(!is.na(merged_ordered[DXgroup,z]), na.rm=T)
    max.raw[z-Ncol]=max(!is.na(merged_ordered[DXgroup,z]), na.rm=T)
  }
  
  #Save raw values
  save(raw.means, sd.raw, n.raw, min.raw, max.raw, file=paste0(outputdir, outputfilenamePrefix,"_DX",DXvalue,".Rdata"))    
}
...
for(APvalue in 0:4){
  ...
  for(z in (Ncol+1):ncol(merged_ordered)){
    ...
    min.raw[z-Ncol]=min(!is.na(merged_ordered[APgroup,z]), na.rm=T)
    max.raw[z-Ncol]=max(!is.na(merged_ordered[APgroup,z]), na.rm=T)
  }
  #Save raw values
  save(raw.means, sd.raw, n.raw, min.raw, max.raw, file=paste0(outputdir, outputfilenamePrefix,"_AP",APvalue,".Rdata"))  
...
save(age.mu, age.sd, age.range, age.mu.dx0, age.sd.dx0, age.range.dx0, age.mu.dx1, age.sd.dx1, 
     ...
     file=paste0(outputdir, "Demographics.Rdata"))
