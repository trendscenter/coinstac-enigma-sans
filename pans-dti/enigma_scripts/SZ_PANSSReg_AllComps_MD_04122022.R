...
messages <- file(paste0(outputdir,"PANSSscriptOut_MD.txt"), open="wt")
...
#Rscript = "SZ_PANSSReg_AllComps_MD_04122022.R"  # This file
dtiFile = paste0(inputdir, "metr_MD.csv")
...
MD <- read.csv(dtiFile, header = T)
Covs <- read.csv(CovarFile, header = T)
PANSS <- read.csv(PANSSFile, header = T)

# Check that all of the required columns are present in the metr_MD.csv files
...
if (anyDuplicated(MD[, c("SubjID")]) != 0) {
    stop(paste0("You have duplicate SubjIDs in your MD  file.\nMake sure there are no repeat SubjIDs.\n"))
}
...
# Merge Covariates with MD
merged_orderedMD = merge(Covs, MD, by = "SubjID")

# Merge with PANSS
merged_orderedMD = merge(merged_orderedMD, PANSS, by = "SubjID", all.x = TRUE)

cat(paste0("The number of SubjIDs in the merged_ordered MD data set is: ", nrow(merged_orderedMD), "\n"))
...
for (phenoName in c("TOT", "NEG", "POS", "GEN", "EXP", "MAP" )) {  # All the possible phenoNames! 
  ...
  genderMod = lm(sexform, data=merged_orderedMD)
...
for (phenoName in c("MD")) {  # brain measure type loop
  
    # extract data subsets using the phenotypic-specific dataset from above
    # Use the total PANSS we created  
    
    dataset = paste0("merged_ordered", phenoName)  # get the right one: MD
