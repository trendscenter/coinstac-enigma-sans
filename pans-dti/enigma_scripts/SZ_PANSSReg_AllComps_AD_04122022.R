...
messages <- file(paste0(outputdir,"PANSSscriptOut_AD.txt"), open="wt")
...
#Rscript = "SZ_PANSSReg_AllComps_AD_04122022.R"  # This file
dtiFile = paste0(inputdir, "metr_AD.csv")
...
AD <- read.csv(dtiFile, header = T)
Covs <- read.csv(CovarFile, header = T)
PANSS <- read.csv(PANSSFile, header = T)

# Check that all of the required columns are present in the metr_AD.csv files
...
if (anyDuplicated(AD[, c("SubjID")]) != 0) {
    stop(paste0("You have duplicate SubjIDs in your AD  file.\nMake sure there are no repeat SubjIDs.\n"))
}
