...
#inputdir="../csv/UCISZ_data_with_PANSS/"
# inputdir="../csv/FBIRN_data_with_PANSS/"
...
#sink(paste0(outputdir,"PANSSscriptOut.txt"), split=TRUE)
...
if (!file.exists(PANSSFile)) {
    stop("PANSS.csv file is missing  please make sure it is located in the specified")
}
...
if (length(which(is.na(cortcolind))) > 0) {
    stop(
        "At least one of the required columns in your Cortical thickness measures file is missing or not in the correct order. Make sure that the column names are spelled exactly as listed in the protocol\n"
    )
}
...
if (length(which(is.na(cortcolind))) > 0) {
    stop(
        "At least one of the required columns in your Cortical surfare area measures file is missing or not in the correct order. Make sure that the column names are spelled exactly as listed in the protocol\n"
    )
}
...
if (length(which(is.na(cortcolind))) > 0) {
    stop(
        "At least one of the required columns in your LandRvolumesn.csv file is missing or not in the correct order. Make sure that the column names are spelled exactly as listed in the protocol\n"
    )
}
...
if (length(which(is.na(colind))) > 0) {
    stop(
        "At least one of the required columns in your Covariates.csv file is missing or not in the correct order. Make sure that the column names are spelled exactly as listed:\nIt is possible that the problem column(s) is: ",
...
if (length(which(is.na(colind))) > 0) {
    stop(
        "At least one of the required columns in your PANSS.csv file is missing or not in the correct order. Make sure that the column names are spelled exactly as listed:\nIt is possible that the problem column(s) is: ",
...
# Put a for loop here for PANSS measures: "TOT", "NEG", "POS", "GEN", "EXP", "MAP"
        ...
        if (nrow(merged_ordered) > (n.sites + 5)) {  # do or not do, there is no try (if enough subjects, run)
            ...
            for (x in (Startcol:Endcol)) {
                ...
                save(r.cort, n.patients, file = paste0(outputdir, "EffectSizes_SZ_only_", predictor,
                  "_withSexAge_", WCov, "_", phenoName, ".Rdata"))
