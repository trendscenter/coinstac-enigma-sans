...
inputdir="../csv/FBIRN2_data_with_SANS/"
...
messages <- file(paste0(outputdir,"SANSscriptOut_AD.txt"), open="wt")
...
if (anyDuplicated(AD[, c("SubjID")]) != 0) {
...
# Merge with SANS
#merged_orderedFA = merge(merged_orderedFA, SANS, by = "SubjID", all.x = TRUE)
merged_orderedAD = merge(merged_orderedAD, SANS, by = "SubjID", all.x = TRUE)
#merged_orderedRD = merge(merged_orderedRD, SANS, by = "SubjID", all.x = TRUE)
#merged_orderedMD = merge(merged_orderedMD, SANS, by = "SubjID", all.x = TRUE)


cat(paste0("The number of SubjIDs in the merged_ordered AD data set is: ", nrow(merged_orderedAD), "\n"))
...
### We want to examine gender effects for SANS: TOT, NEG, POS, GEN, EXP, and MAP
...
for (phenoName in c("SANSSum", "SANSMAP", "SANSEXP",
  ...
  genderMod = lm(sexform, data=merged_orderedAD)
...
for (phenoName in c("AD")) {  # brain measure type loop
 
    # extract data subsets using the phenotypic-specific dataset from above
    # Use the total SANS we created  
    
    dataset = paste0("merged_ordered", phenoName)  # get the right one: AD
